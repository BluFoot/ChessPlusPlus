...
class Position final
{
  public:
     ...
     */
    friend auto operator==(Position const& a, Position const& b) noexcept -> typename std::enable_if<std::is_integral<T>::value,
                                                                                                     bool>::type {
 ...
 */
template<typename T>
auto operator<<(std::ostream& os, Position<T> const& p) noexcept -> typename std::enable_if<
    std::is_integral<T>::value && std::is_same<T, typename MakeSigned<T>::type>::value, std::ostream&>::type {
    //cast in case of char
    return os << '(' << static_cast<std::intmax_t>(p.x) << ", " << static_cast<std::intmax_t>(p.y) << ')';
}
/**
 * Serializes a position to a stream in the format "(x, y)".
...
auto operator<<(std::ostream& os, Position<T> const& p) noexcept -> typename std::enable_if<
    std::is_integral<T>::value && !std::is_same<T, typename MakeSigned<T>::type>::value, std::ostream&>::type {
    //cast in case of char
    return os << '(' << static_cast<std::uintmax_t>(p.x) << ", " << static_cast<std::uintmax_t>(p.y) << ')';
}
/**
 * Serializes a position to a stream in the format "(x, y)".
 ...
 */
template<typename T>
auto operator<<(std::ostream& os, Position<T> const& p) noexcept -> typename std::enable_if<!std::is_integral<T>::value,
                                                                                            std::ostream&>::type {
    return os << '(' << p.x << ", " << p.y << ')';
}
}
