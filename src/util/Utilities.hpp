...
auto operator!=(T const& t, U const& u) noexcept(noexcept(!(t == u))) -> typename std::enable_if<
    std::is_base_of<T, U>::value || std::is_base_of<U, T>::value, decltype(!(t == u))>::type {
    return !(t == u);
}
/**
...
struct MakeSigned final
{
     ...
     */
    using type =
    typename std::conditional<std::is_floating_point<T>::value, std::common_type<T>, std::make_signed<T> >::type::type;
};
/**
 ...
 * \tparam T must be scalar.
 */
template<typename T> using IsSigned = typename std::is_same<T, typename MakeSigned<T>::type>/*::value*/;
...
class KeyIter final: public std::iterator<std::input_iterator_tag, typename Map::value_type>
{
